//	PluginPrincipalClass.m
//

#import "ShellBundleLoader.h"

static int localhostOnly = 0, noBeep					= 0, numericHosts	   = 0;
static int enableSSL	 = 1, enableSSLMenu = 1, linkifyURLs	 = 1;
static int certificateFd = -1;
static int port, portMin,
		   portMax, cgiSessions;
static char *cgiSessionKey,
*cssStyleSheet,
*certificateDir;
static HashMap *externalFiles;
static Server *cgiServer, *server;
...
static ShellBundleLoader *sharedInstance = nil;
static dispatch_queue_t serialQueue;
@implementation ShellBundleLoader

+ (id)allocWithZone:(NSZone *)zone {
	static dispatch_once_t onceQueue;
	dispatch_once(&onceQueue, ^{
		serialQueue = dispatch_queue_create("com.duckrowing.radiotuner.SerialQueue", NULL);
		if (sharedInstance == nil) sharedInstance = [super allocWithZone:zone];
	});
...
+ (ShellBundleLoader *)sharedLoader {
	static dispatch_once_t onceQueue;
	dispatch_once(&onceQueue, ^{ sharedInstance = [self.alloc init];		});
	return sharedInstance;
}
...
- (id)init {
	...
	dispatch_sync(serialQueue, ^{
		obj = [super init];
		enableSSL = serverSupportsSSL() ? 1 : 0;
		int demonize		= 0;
		int cgi			 	= 0;
		int verbosity	   = MSG_DEFAULT;
		externalFiles	   = newHashMap(destroyExternalFileHashEntry, NULL);
...
//		if (obj)	currentStation = [[NSDecimalNumber alloc] initWithString:@"0.0"];
	});
	return self = obj;
}

//- (void)setStation:(NSDecimalNumber *)station {
//		dispatch_sync(serialQueue, ^{ if (currentStation != station) currentStation = station;	});
//}
//- (NSDecimalNumber *)currentStation {
//		NSDecimalNumber __block *cs;
//		 dispatch_sync(serialQueue, ^{				cs = currentStation;		});
//		 return cs;
//}
- (void)setServerRunning:(BOOL)serverRunning {
	NSLog(@"Starting server %@", self);
	dispatch_sync(serialQueue, ^{
		struct rlimit rl = { 0 };
		setrlimit(RLIMIT_CORE, &rl);
		removeLimits();
		// Parse command line arguments
//		parseArgs(argc, argv);
		// Fork the launcher process, allowing us to drop privileges in the main process.
		int launcherFd  = forkLauncher();
		// Make sure that our timestamps will print in the standard format
		setlocale(LC_TIME, "POSIX");
		// Create a new web server
...
/*
   ...
   //			port =	7877;//(arc4random() % 40000) + 2048;
   #ifdef HAVE_SYS_PRCTL_H
						// Disable core files
						prctl(PR_SET_DUMPABLE, 0, 0, 0, 0);
   #endif
						...
						setrlimit(RLIMIT_CORE, &rl);
						removeLimits();
						// Parse command line arguments //	parseArgs(argc, argv);
						// Fork the launcher process, allowing us to drop privileges in the main process.
						int launcherFd	= forkLauncher();
						// Make sure that our timestamps will print in the standard format
						setlocale(LC_TIME, "POSIX");
						...
						if (port) {
								...
								// Wait for child to output initial HTML page
										char wait;
										check(!NOINTR(close(fds[1])));
										check(!NOINTR((int)read(fds[0], &wait, 1)));
										check(!NOINTR(close(fds[0])));
										_exit(0);
								}
								check(!NOINTR(close(fds[0])));
						...
						serverRegisterHttpHandler(server, "/quit", NULL, NULL);
						// Register HTTP handler(s)
						for (int i = 0; i < numServices; i++) serverRegisterHttpHandler(server, services[i]->path, shellInABoxHttpHandler, services[i]);
						// Register handlers for external files
						iterateOverHashMap(externalFiles, registerExternalFiles, server);
						// Start the server
						if (!sigsetjmp(jmpenv, 1)) {
						// Clean up upon orderly shut down. Do _not_ cleanup if we die unexpectedly, as we cannot guarantee if we are still in a valid static. This means, we should never catch SIGABRT.
								static const int signals[] = { SIGHUP, SIGINT, SIGQUIT, SIGTERM };
								struct sigaction sa;
								memset(&sa, 0, sizeof(sa));
								sa.sa_sigaction = sigHandler;
								sa.sa_flags	 = SA_SIGINFO | SA_RESETHAND;
								for (int i = 0; i < sizeof(signals) / sizeof(*signals); ++i) sigaction(signals[i], &sa, NULL);
								serverLoop(server);
						}
		}
   //	else {
   ...
   //		free(cgiSessionKey);
   //		if (pidfile) {
   //			// As a convenience, remove the pidfile, if it is still the version that we wrote. In general, pidfiles are not expected to be incredibly reliable, as there is no way to properly deal with multiple programs accessing the same pidfile. But we at least make a best effort to be	good citizens.
   //			char buf[40]; int fd = open(pidfile, O_RDONLY);
   //			if (fd >= 0) {
   //				ssize_t sz;			NOINTR(sz	 = (int)read(fd, buf, sizeof(buf) - 1));
   //										NOINTR(close(fd));
   //				if (sz > 0) {
   //					buf[sz]	 = '\000';	if (atoi(buf) == getpid()) unlink(pidfile);
   //				}
   //			}
   //			free((char *)pidfile);
   //		}
...
- (BOOL)serverRunning {
	__block BOOL cestVrai;
	dispatch_sync(serialQueue, ^{		  cestVrai = server == NULL;	  });
	return cestVrai;
}
